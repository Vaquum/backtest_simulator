...
- Bootstrap M1 end-to-end simulator + M2 honesty-hardening (closes #10 as slice; full Closeout tracked on the merge commit).
...
- Part 2 CAPITAL honesty: real `ValidationPipeline` wired (CAPITAL=real, others=`_allow` stub), 4-step `CapitalController` lifecycle driven by `CapitalLifecycleTracker` (`check_and_reserve → send_order → order_ack → order_fill`), `assert_conservation` enforces INV-1a / INV-1b / INV-2 / INV-3 after every event. SELL-as-close short-circuits the CAPITAL stage to avoid double-commit on exits; pinned by `tests/honesty/test_sell_close_semantics.py`.
...
- Scope deferred to follow-up M-slices (documented in PR body): own-order market impact, passive maker-fill realism, gap-risk on declared stops, statistical honesty (CPCV / DSR / PBO / SPA), ledger parity vs Praxis, sanity baselines (buy-hold / random / zero-trade / maker-no-fill / over-trading), prescient + inverse-prescient + shuffle-bars tests.
