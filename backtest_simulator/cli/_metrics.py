...
class Trade:
    ...
    def __init__(self, coid: str, side: str, qty: str, price: str, fee: str, fee_asset: str, ts: str) -> None:
...
def pair_metrics(pair: tuple[Trade, Trade], declared_stop: Decimal | None) -> tuple[Decimal, Decimal, Decimal | None]:
    buy, sell = pair
    qty = buy.qty
...
def print_run(perm_id: int, day_label: str, trades: list[Trade], declared_stops: dict[str, Decimal], *, n_intents: int=0, n_fills: int=0, n_pending: int=0, n_rejects: int=0, slippage_cost_bps: Decimal | None=None, slippage_n_samples: int=0, slippage_n_excluded: int=0, slippage_predict_vs_realised_gap_bps: Decimal | None=None, slippage_n_uncalibrated_predict: int=0, slippage_n_predicted_samples: int=0, n_limit_orders_submitted: int=0, n_limit_filled_full: int=0, n_limit_filled_partial: int=0, n_limit_filled_zero: int=0, n_limit_marketable_taker: int=0, maker_fill_efficiency_p50: Decimal | None=None, market_impact_realised_bps: Decimal | None=None, market_impact_n_samples: int=0, market_impact_n_flagged: int=0, market_impact_n_uncalibrated: int=0) -> None:
    ...
    if losers:
        ...
        pf_str = '—'
    r_mean_str = fmt_dec(sum(r_mults, Decimal('0')) / len(r_mults), 2) if r_mults else '—'
    if slippage_cost_bps is None:
        slip_str = 'slip off'
    else:
        slip_str = f'slip {fmt_dec(slippage_cost_bps, 2)}bp n={slippage_n_samples}/excl={slippage_n_excluded}'
        if slippage_predict_vs_realised_gap_bps is not None and slippage_n_predicted_samples > 0:
            slip_str += f' gap {fmt_dec(slippage_predict_vs_realised_gap_bps, 2)}bp (n_predict={slippage_n_predicted_samples}'
            slip_str += ')'
    if n_limit_orders_submitted > 0:
        eff_str = 'n/a' if maker_fill_efficiency_p50 is None else f"{(maker_fill_efficiency_p50 * Decimal('100')).quantize(Decimal('0.1'))}%"
        maker_str = f'  maker n={n_limit_orders_submitted} full/part/zero={n_limit_filled_full}/{n_limit_filled_partial}/{n_limit_filled_zero}  mkt_taker={n_limit_marketable_taker}  eff_p50={eff_str}'
    else:
        maker_str = ''
    if market_impact_realised_bps is not None and (market_impact_n_samples > 0 or market_impact_n_uncalibrated > 0):
        impact_core = f'  imp {fmt_dec(market_impact_realised_bps, 2)}bp n={market_impact_n_samples}/flagged={market_impact_n_flagged}'
    ...
    activity_str = f" ({', '.join(activity_extras)})" if activity_extras else ''
    print(f'   perm {perm_id:<4}  {day_label}  trades {n_trades}{activity_str}  PF {pf_str:<6}  R̄ {r_mean_str:<7}  DD {fmt_dec(-max_dd_pct, 2)}%  total {fmt_dec(total_pct, 2)}%  {slip_str}{maker_str}{impact_str}')
    for buy, sell in pairs:
        declared = declared_stops.get(buy.client_order_id)
        ...
        print(f'     {in_tm} → {out_tm}   BUY {fmt_price(buy.price):>10} → SELL {fmt_price(sell.price):>10}   {fmt_dec(ret_pct, 2)}%   {r_str}')
