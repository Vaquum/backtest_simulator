"""`OverTradingStrategy` — flip-flops every signal; the fee-eats-all-edge baseline."""

# Slice #17 Task 4: a Nexus `Strategy` that toggles long/flat on every
# single `on_signal` (independent of `_preds`), using a tight stop_bps
# so the R-denominator is small. Over a window with realistic fees
# this MUST produce mean R/trade < 0 and profit factor < 1: any edge
# from price drift is dwarfed by the round-trip fee on every flip.
#
# Used by tests/honesty/test_sanity_over_trading.py to pin the
# "over-trading is unprofitable by construction" identity. If a
# regression makes our fee model under-charge — or if walk_trades
# starts giving free price improvement on every fill — this baseline
# stops failing and the simulator is silently more generous than
# live.
...
class OverTradingStrategy(_StrategyBase):
    """Toggle long/flat on every signal; ignore preds entirely."""
