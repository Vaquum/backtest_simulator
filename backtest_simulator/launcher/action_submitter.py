...
from praxis.core.domain.single_shot_params import SingleShotParams as _PraxisSingleShotParams
...
def _install_nexus_enum_shim() -> None:
    ...
    for nexus_em in _NexusExecutionMode:
        ...
        synonym_set: frozenset[Enum] = frozenset({order_name[n] for n in names if n in order_name} | {nexus_order_name[n] for n in names if n in nexus_order_name})
...
_ACTION_TYPE_TO_VALIDATION_ACTION: dict[ActionType, ValidationAction] = {ActionType.ENTER: ValidationAction.ENTER, ActionType.EXIT: ValidationAction.EXIT, ActionType.MODIFY: ValidationAction.MODIFY, ActionType.ABORT: ValidationAction.ABORT}
_PRAXIS_ENUM_BY_FIELD: dict[str, type[Enum]] = {'side': _PraxisOrderSide, 'order_type': _PraxisOrderType, 'execution_mode': _PraxisExecutionMode, 'maker_preference': _PraxisMakerPreference, 'stp_mode': _PraxisSTPMode}
...
def _convert_enum_field(field_name: str, value: object) -> object:
    ...
    try:
        ...
        _log.debug('enum convert skipped: %s=%r has no counterpart in %s', field_name, value, praxis_enum.__name__)
        return value
...
def _coerce_optional_decimal(value: object) -> Decimal | None:
    ...
    msg = f'single-shot price field: expected Decimal|str|int|float|None, got {type(value).__name__}'
    raise TypeError(msg)
...
def _wrap_single_shot_params(attrs: dict[str, object]) -> None:
    ...
    stop_required = {_PraxisOT.STOP, _PraxisOT.STOP_LIMIT, _PraxisOT.TAKE_PROFIT, _PraxisOT.TP_LIMIT, _PraxisOT.OCO}
    order_type_uses_stop = attrs.get('order_type') in stop_required
    stop_price_for_praxis = extracted['stop_price'] if order_type_uses_stop else None
    attrs['execution_params'] = _PraxisSingleShotParams(price=extracted['price'], stop_price=stop_price_for_praxis, stop_limit_price=extracted['stop_limit_price'])
    attrs['declared_stop_price'] = declared_stop_price

def _to_praxis_enums(cmd: TradeCommand) -> TradeCommand:
    attrs: dict[str, object] = {field.name: _convert_enum_field(field.name, getattr(cmd, field.name)) for field in dataclasses.fields(cmd)}
...
def build_action_submitter(bindings: SubmitterBindings, *, on_reservation: ReservationHook | None=None, on_submit: SubmitHook | None=None) -> Callable[[list[Action], str], None]:

    def _submit(actions: list[Action], strategy_id: str) -> None:
        for raw_action in actions:
            action = _maybe_refresh_limit_to_touch(raw_action, touch_provider=bindings.touch_provider, tick_provider=bindings.tick_provider)
            _log.info('action_submit: type=%s direction=%s size=%s', action.action_type.value, action.direction.name if action.direction else None, action.size)
...
def _submit_translated(bindings: SubmitterBindings, strategy_id: str, action: Action) -> tuple[str, ValidationDecision, ValidationRequestContext]:
    ...
    if action.direction == OrderSide.SELL:
        ...
        cmd = translate_to_trade_command(action, context, decision, bindings.nexus_config, datetime.now(UTC))
        cmd = _to_praxis_enums(cmd)
        command_id = bindings.praxis_outbound.send_command(cmd)
        _log.info('backtest action submitted (SELL close — pipeline bypassed)', extra={'strategy_id': strategy_id, 'action_type': action.action_type.value, 'command_id': command_id})
...
def _build_context(*, config: NexusInstanceConfig, state: InstanceState, strategy_id: str, action: Action, strategy_budget: Decimal) -> ValidationRequestContext:
    ...
    base_notional = order_size * action.reference_price if action.reference_price is not None else Decimal('0')
    order_notional = base_notional * (Decimal('1') + NOTIONAL_RESERVATION_BUFFER)
    estimated_fees = order_notional * _FEE_ESTIMATE_RATE
    return ValidationRequestContext(strategy_id=strategy_id, order_notional=order_notional, estimated_fees=estimated_fees, strategy_budget=strategy_budget, state=state, config=config, action=_ACTION_TYPE_TO_VALIDATION_ACTION[action.action_type], symbol=symbol, order_side=_resolve_side(action), order_size=action.size, trade_id=action.trade_id or f'bts-{uuid.uuid4().hex[:12]}', command_id=action.command_id or f'bts-cmd-{uuid.uuid4().hex[:12]}', current_order_notional=None)
