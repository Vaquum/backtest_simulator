...
- [ ] **Task 29.** Wire `AtrSanityGate` into `action_submitter._check_declared_stop`. The gate (`backtest_simulator/honesty/atr.py`) ships standalone with its MVC test pinning the `(entry, stop, atr) → decision` contract, but `_check_declared_stop` (action_submitter.py:518) still accepts any non-blank BUY stop and R/PRICE stages stay `_allow`. A 1 bp `stop_price` reaches Praxis. The wiring needs a decision-time ATR source (precompute over `atr_window_seconds` ending at signal time) and an integration test that the command is denied before `send_command` rather than after. Codex Task 10 round 2 surfaced this; the standalone-correctness test stands, Task 29 closes the wiring loop.
...
- [ ] **Task 31.** Wire `MarketImpactModel` into the simulator's pre-fill check and propagate the realised impact bps + flag to `bts run --output-format json`. The model (`backtest_simulator/honesty/market_impact.py`) ships standalone with its MVC test pinning the (qty/volume) scaling and flag threshold on the real BTCUSDT tape, but the venue/launcher pipeline never calls `evaluate()` and `bts run --output-format json` still emits `market_impact_realised_bps: None`. Task 31 plumbs (a) calibrate-once-per-run from the same feed window, (b) `evaluate()` per ENTER action — reject when `flag=True` and the operator has set the strict policy, (c) running aggregate of impact bps surfaced in the JSON report.
...
- [ ] **Task 33.** Wire `MakerFillModel` into the launcher's LIMIT-OPEN handling. The model (`backtest_simulator/honesty/maker_fill.py`) ships standalone with its MVC test pinning queue-position + partial-fill semantics on the real trade tape (Task 14) and the far-away-no-fill sanity (Task 15), but `walk_trades` does not yet invoke it for LIMIT orders — the existing path uses an O(1) limit-take check that does not honor queue position. Task 33 plumbs (a) the model's calibration into the launcher's bootstrap, (b) `evaluate()` per resting LIMIT order on every clock tick that drains the trades-in-window, (c) the resulting `ImmediateFill` list converted to `FillResult` legs the existing reporting expects.
