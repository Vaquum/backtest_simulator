...
- [ ] **Task 32.** Wire `SlippageModel` into `venue/fills.py::walk_trades` and propagate the realised bps to `bts run --output-format json`. The model (`backtest_simulator/honesty/slippage.py`) ships standalone with its MVC test pinning the calibration's directional and symmetric properties on the real BTCUSDT trade tape, but `walk_trades` continues returning tape VWAP / tick prices directly and `bts run --output-format json` still emits `slippage_realised_bps: None`. Task 32 plumbs (a) calibrate-once-per-run from the same feed window, (b) `apply()` per fill leg adjusting `fill_price` by the side-signed bps (`fill_price = mid * (1 + bps / 10000)`), (c) running aggregate of realised slippage_bps surfaced in the JSON report. Codex Task 12 round 3 surfaced this.
